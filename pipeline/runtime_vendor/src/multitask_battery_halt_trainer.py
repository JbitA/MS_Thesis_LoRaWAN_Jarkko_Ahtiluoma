#!/usr/bin/env python3
"""
FILE STORY — ``multitask_battery_halt_trainer.py``
================================================

**Role.** Older/alternate trainer entry (superseded by ``*_bundle.py`` for node_analysis runs).

**Connects.** Same stack as bundle; kept for parity with upstream thesis code.
...
# Reuse telemetry_sequence_pipeline for labels, windows, and comparability with legacy baselines.
_SRC = Path(__file__).resolve().parent
if str(_SRC) not in sys.path:
    sys.path.insert(0, str(_SRC))

import telemetry_sequence_pipeline as telemetry
from experiments.neural_count_auxiliary_heads import build_count_aux_models
from experiments.patch_mixer_transformer import PatchMixerTransformerHeadV5
from experiments.regime_gated_battery_heads import build_regime_gated_models
from experiments.stratified_halt_batch_sampler import HaltStratifiedBatchSampler
from experiments.discrete_survival_halt_targets import survival_cumulative_targets
from experiments.stochastic_weight_averaging import create_swa_averager, load_averaged_into_model, swa_update

BASE_DIR = Path(__file__).resolve().parents[2]
OUT_TABLES = BASE_DIR / "thesis_pipeline" / "outputs" / "tables"
OUT_FIGS = BASE_DIR / "thesis_pipeline" / "outputs" / "figures"
OUT_REPORTS = BASE_DIR / "thesis_pipeline" / "outputs" / "reports"
...
class LSTMHead(nn.Module):
    def __init__(self, in_dim: int, hidden_dim: int = 64, n_battery_out: int = 1, n_halt_out: int = 1):
        super().__init__()
        self.n_battery_out = int(n_battery_out)
        self.n_halt_out = int(n_halt_out)
        self.lstm = nn.LSTM(input_size=in_dim, hidden_size=hidden_dim, num_layers=2, batch_first=True, dropout=0.1)
        self.reg = nn.Linear(hidden_dim, self.n_battery_out)
        self.cls = nn.Linear(hidden_dim, self.n_halt_out)

    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        h, _ = self.lstm(x)
        z = h[:, -1, :]
        r = self.reg(z)
        if self.n_battery_out == 1:
            r = r.squeeze(-1)
        c = self.cls(z)
        if self.n_halt_out == 1:
            c = c.squeeze(-1)
        return r, c
...
class GRUHead(nn.Module):
    def __init__(self, in_dim: int, hidden_dim: int = 64, n_battery_out: int = 1, n_halt_out: int = 1):
        ...
        self.n_battery_out = int(n_battery_out)
        self.n_halt_out = int(n_halt_out)
        self.gru = nn.GRU(input_size=in_dim, hidden_size=hidden_dim, num_layers=2, batch_first=True, dropout=0.1)
        self.reg = nn.Linear(hidden_dim, self.n_battery_out)
        self.cls = nn.Linear(hidden_dim, self.n_halt_out)
...
class TransformerHeadV5(nn.Module):
    """Shallower encoder, norm_first — same inductive bias as V3 Transformer."""
    ...
    def __init__(
        ...
        self.reg = nn.Linear(d_model, self.n_battery_out)
        self.cls = nn.Linear(d_model, self.n_halt_out)
...
class TCNHead(nn.Module):
    """Local temporal patterns via stacked 1D convolutions; last time step pooled for heads."""
    ...
    def __init__(
        ...
        for c_out in channels:
            layers.append(nn.Conv1d(c_in, c_out, kernel_size, padding=pad))
            layers.append(nn.ReLU())
            layers.append(nn.BatchNorm1d(c_out))
            c_in = c_out
        self.conv = nn.Sequential(*layers)
        self.reg = nn.Linear(c_in, self.n_battery_out)
        self.cls = nn.Linear(c_in, self.n_halt_out)
...
def run_train(
    ...
    swa_k = max(0, int(swa_last_epochs))
    averager = create_swa_averager(model) if swa_k > 0 else None
    ...
    for ep in range(epochs):
        ...
        if averager is not None:
            start_ep = max(0, epochs - swa_k) if swa_k <= epochs else 0
            if ep >= start_ep:
                swa_update(averager, model)
    if averager is not None:
        load_averaged_into_model(model, averager)
...
def fit_temperature_min_brier(logits: np.ndarray, y: np.ndarray) -> float:
    """Scalar T>0 minimizing Brier on validation; monotonic in logits so ranking (AUC) unchanged on test."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Multitask battery + halt trainer: multi-backbone ensemble stack with calibration, threshold policy, optional survival/count/regime/SWA/patch-Transformer options."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="B08: SWA averaging over the last K epochs (0=off). Final weights are the SWA average, not best-val snapshot.",
    )
    parser.add_argument(
        "--patch-len",
        type=int,
    ...
    if cohort_csv:
        ...
        allow = cdf[col].astype(str).str.strip().tolist()
        print(f"  Using device allowlist from {cp} ({len(allow)} rows).")
    ...
    if args.artifact_stem is None:
        ...
        if args.swa_last_epochs > 0:
            artifact_stem = f"{artifact_stem}_swa{args.swa_last_epochs}"
        if args.patch_len > 0:
    ...
    survival_mode = args.halt_mode == "survival"
    regime_gated = args.battery_regime_heads != "single"
    count_aux_mode = args.count_aux_weight > 0.0
    if quantile_mode and ordinal_mode:
        raise SystemExit("Choose one of --battery-loss quantile or ordinal.")
    if survival_mode and ordinal_mode:
        raise SystemExit("--halt-mode survival is not supported with --battery-loss ordinal.")
    if regime_gated and (quantile_mode or ordinal_mode):
        raise SystemExit("--battery-regime-heads gated_* requires --battery-loss mse (B05).")
    if args.weighted_sampler and args.halt_stratified_batches:
        raise SystemExit("Use either --weighted-sampler or --halt-stratified-batches, not both (B06).")
    if count_aux_mode and (quantile_mode or ordinal_mode or survival_mode or regime_gated):
        raise SystemExit(
            "--count-aux-weight requires MSE battery, binary halt, single battery head "
            "(no quantile/ordinal/survival/regime-gated)."
        )
    if args.patch_len > 0:
        if args.seq_len % args.patch_len != 0:
            raise SystemExit("--patch-len must divide --seq-len (B09).")
        if count_aux_mode or regime_gated:
            raise SystemExit("--patch-len is only supported with single battery/halt heads (B09).")
    ...
    y_surv_tr = survival_cumulative_targets(seq.y_halt_dt[tr_idx], args.halt_horizon_days)
    y_surv_va = survival_cumulative_targets(seq.y_halt_dt[va_idx], args.halt_horizon_days)
    y_surv_te = survival_cumulative_targets(seq.y_halt_dt[te_idx], args.halt_horizon_days)
    n_halt_out = int(args.halt_horizon_days) + 1 if survival_mode else 1
    yc_tr = yc_va = yc_te = None
    if count_aux_mode:
        assert seq.y_next_count is not None
        yc_tr = seq.y_next_count[tr_idx].astype(np.float32)
        yc_va = seq.y_next_count[va_idx].astype(np.float32)
        yc_te = seq.y_next_count[te_idx].astype(np.float32)
    ...
    if ordinal_mode:
        ...
        train_ds = TensorDataset(
            ...
            torch.tensor(yh_tr),
            torch.tensor(lb_tr),
            torch.tensor(yc_tr, dtype=torch.float32),
        )
    else:
        train_ds = TensorDataset(
            torch.tensor(x_tr),
    ...
    stratified_ok = int(np.sum(yh_tr > 0.5)) > 0 and int(np.sum(yh_tr <= 0.5)) > 0
    if args.halt_stratified_batches and not stratified_ok:
        print("Warning: train split lacks both halt± rows; falling back to shuffle (B06).")
    if args.halt_stratified_batches and stratified_ok:
        batch_sampler = HaltStratifiedBatchSampler(
            yh_tr,
            batch_size=args.batch_size,
            pos_fraction=args.halt_stratified_pos_fraction,
            seed=42,
        )
    ...
    if ordinal_mode:
        ...
        val_ds = TensorDataset(
            ...
            torch.tensor(yh_va),
            torch.tensor(lb_va),
            torch.tensor(yc_va, dtype=torch.float32),
        )
        test_ds = TensorDataset(
            torch.tensor(x_te),
            torch.tensor(yb_te),
    ...
    in_dim = x_tr.shape[-1]
    if count_aux_mode:
        models = build_count_aux_models(
            in_dim=in_dim,
            n_batt_out=n_batt_out,
            n_halt_out=n_halt_out,
            include_tcn=not args.no_tcn,
        )
    elif regime_gated:
        gm = "learned" if args.battery_regime_heads == "gated_learned" else "hand_voltage"
        models = build_regime_gated_models(
            in_dim=in_dim,
            n_batt_out=n_batt_out,
            n_halt_out=n_halt_out,
            gate_mode=gm,
            batt_feat_mu=batt_feat_mu,
            batt_feat_std=batt_feat_std,
            include_tcn=not args.no_tcn,
            hand_voltage_mid=args.regime_hand_v_mid,
            hand_voltage_tau=args.regime_hand_v_tau,
        )
    else:
        models = {
        ...
        if args.patch_len > 0:
            models["transformer"] = PatchMixerTransformerHeadV5(
                in_dim=in_dim,
                seq_len=int(args.seq_len),
                patch_len=int(args.patch_len),
                n_battery_out=n_batt_out,
                n_halt_out=n_halt_out,
            )
    ...
    for name, model in models.items():
        ...
        model, epochs_done = run_train(
            ...
            count_aux_weight=args.count_aux_weight if count_aux_mode else 0.0,
            swa_last_epochs=int(args.swa_last_epochs),
        ...
        if survival_mode:
            tb_pred, th_prob, tb_true, th_true, Z_te, P_te = predict_survival_export(
                model,
                test_loader,
                device,
                battery_residual=residual_mode,
                battery_quantile=False,
                battery_ordinal=False,
                bin_centers=None,
            )
            tb_q, tb_bin = None, None
        else:
        ...
        lg_val = collect_halt_logits(model, val_loader, device, survival_last_logit=survival_mode)
        lg_te = collect_halt_logits(model, test_loader, device, survival_last_logit=survival_mode)
        t_scale = fit_temperature_min_brier(lg_val, vh_true)
        z_te = lg_te / t_scale
        p_te_scaled = 1.0 / (1.0 + np.exp(-np.clip(z_te, -60.0, 60.0)))
        ...
        if survival_mode:
            K = int(Z_te.shape[1])
            for k in range(K):
                pred_cols[f"y_true_halt_cum_k{k}"] = Z_te[:, k]
                pred_cols[f"y_pred_halt_cum_k{k}"] = P_te[:, k]
            pred_cols["y_halt_dt_days"] = seq.y_halt_dt[te_idx]
            rows_k: List[Dict[str, float]] = []
            for k in range(K):
                yk = Z_te[:, k].astype(int)
                pk = P_te[:, k].astype(np.float64)
                prev = float(np.mean(yk))
                br = float(brier_score_loss(yk, pk))
                try:
                    auc_k = float(roc_auc_score(yk, pk)) if len(np.unique(yk)) > 1 else float("nan")
                except ValueError:
                    auc_k = float("nan")
                rows_k.append({"cum_day_k": k, "brier": br, "auc": auc_k, "prevalence": prev})
            pd.DataFrame(rows_k).to_csv(
                OUT_TABLES / f"{artifact_stem}_{name}_survival_per_k_metrics.csv", index=False
            )
